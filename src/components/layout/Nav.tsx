...
export default function Nav() {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 px-6 md:px-10 py-6 flex items-center justify-between mix-blend-difference transition-all duration-300 ${scrolled ? "backdrop-blur-[5px]" : ""}`}
      >
        <Link
          to="/"
          className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.22em] font-medium"
        >
          <span className="inline-block w-2 h-2 rounded-full bg-[var(--orange)] spin-slow" />
          <Scramble>DEBARGHA</Scramble>
          <span className="font-editorial italic text-base lowercase ml-0.5 normal-case tracking-normal text-[var(--acid)]">
            Moriarty
          </span>
        </Link>
        ...
        >
          {NAV_LINKS.map((i) => (
            <li key={i.to}>
              <Link
                to={i.to}
                className={`relative group transition-colors ${location.pathname === i.to ? "text-[var(--acid)]" : "hover:text-[var(--acid)]"}`}
              >
                <Scramble>{i.l}</Scramble>
                <span
                  className={`absolute -bottom-1 left-0 w-full h-[1px] bg-current transition-transform duration-300 origin-left ${location.pathname === i.to ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`}
                />
              </Link>
            </li>
          ))}
      ...
      >
        <div className="absolute top-0 left-0 right-0 px-6 py-6 flex justify-between items-center border-b border-[var(--bone)]/10">
          <Link
            to="/"
            className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.22em] font-medium text-white"
          >
        ...
        <div className="flex flex-col justify-center h-full px-6 gap-6">
          {NAV_LINKS.map((i, idx) => (
            <Link
              key={i.to}
              to={i.to}
              className={`relative w-fit font-display text-5xl transition-colors group ${location.pathname === i.to ? "text-[var(--acid)]" : "text-white hover:text-[var(--acid)]"}`}
              style={{ transitionDelay: menuOpen ? `${idx * 0.05}s` : "0s" }}
            >
              {i.l}
              <span
                className={`absolute -bottom-2 left-0 w-full h-[2px] bg-current transition-transform duration-300 origin-left ${location.pathname === i.to ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`}
              />
            </Link>
          ))}
