  ...
  return (
    <div>
      <div className="font-display display-thin text-7xl md:text-8xl text-[var(--acid)] tracking-[-0.04em]">
        <span ref={mergeRefs(ref)} className="number-flip">
          0
        </span>
      </div>
...
export default function StudioTease() {
  ...
  const bio =
    "We are a small, senior studio — six people, one room in Berlin, one kiln in the basement. Every project is led by a partner, every line of code is written in-house.";
    ...
    <section className="px-6 md:px-10 py-28 md:py-40 border-t border-[var(--bone)]/15">
      <div className="grid grid-cols-12 gap-8">
        <div className="col-span-12 md:col-span-3">
          ...
          <Magnetic>
            <Link to="/studio" className="chip shine">
        ...
        <div className="col-span-12 md:col-span-9">
          <h2
            ref={bioRef}
            className="reveal-group font-display display-light text-[7vw] md:text-[3.5vw] leading-[1.1] tracking-[-0.03em]"
          >
            {bio.split(" ").map((w, i) => (
              <span key={i} className="inline-block overflow-hidden mr-[0.22em] align-bottom">
                <span
                  className={`reveal-fade inline-block ${[4, 8, 14, 21, 26].includes(i) ? "font-editorial italic" : ""}`}
