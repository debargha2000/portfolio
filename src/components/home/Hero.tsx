      ...
      <div ref={skew} className="relative z-10 mt-auto">
        <div className="text-white hero-morph-text">
          <div className="overflow-hidden">
            <div className={`font-display display-thin text-[18vw] sm:text-[16vw] md:text-[13vw] lg:text-[11vw] leading-[0.9] tracking-[-0.04em] ml-[2%] sm:ml-[4%] lg:ml-[6%] transform transition-transform duration-[1200ms] ease-[cubic-bezier(0.19,1,0.22,1)] delay-[200ms] ${mounted ? 'translate-y-0' : 'translate-y-[110%]'}`}>
              Designing<span className="font-editorial text-[var(--acid)] black-stroke"> the</span>
            </div>
          </div>
          <div className="overflow-hidden">
            <div className={`font-display display-light text-[18vw] sm:text-[16vw] md:text-[13vw] lg:text-[11vw] leading-[0.9] tracking-[-0.04em] ml-[8%] sm:ml-[15%] lg:ml-[22%] transform transition-transform duration-[1200ms] ease-[cubic-bezier(0.19,1,0.22,1)] delay-[450ms] ${mounted ? 'translate-y-0' : 'translate-y-[110%]'}`}>
              <span className="font-editorial text-[#ff5f00] white-stroke">quiet </span>spaces
            </div>
          </div>
          <div className="overflow-hidden">
            <div className={`font-editorial italic text-[16.5vw] sm:text-[14.5vw] md:text-[11.5vw] lg:text-[9.5vw] leading-[0.95] tracking-[-0.02em] ml-[12%] sm:ml-[25%] lg:ml-[40%] transform transition-transform duration-[1200ms] ease-[cubic-bezier(0.19,1,0.22,1)] delay-[700ms] ${mounted ? 'translate-y-0' : 'translate-y-[110%]'}`}>
              between <span className="text-[#ff5f00] white-stroke">noise</span><span className="text-[var(--acid)] white-stroke">.</span>
            </div>
          </div>
        </div>

        <div className={`mt-8 md:mt-12 flex flex-col md:flex-row md:items-end justify-between gap-6 text-white transform transition-all duration-[800ms] ease-[cubic-bezier(0.19,1,0.22,1)] delay-[1300ms] ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
          <WordReveal
