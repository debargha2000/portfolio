...
const ProjectImage = lazy(() => import("../work/ProjectImage"));
import { Magnetic, Scramble } from "../../components/motion/Motion";
import { useTilt, useReveal } from "../../hooks/motionUtils";
  ...
  return (
    <Link
      ...
      className={`tilt-card group relative overflow-hidden block ${
        i === 0
          ? "md:col-span-12 lg:col-span-7 aspect-[4/3]"
          : i === 1
            ? "md:col-span-12 lg:col-span-5 aspect-[4/3]"
    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col justify-end p-6 md:p-10">
        <div className="tiny text-[var(--acid)] mb-3">
          ({p.index}) · {p.year}
        </div>
        ...
        <div className="font-sans text-sm font-light text-white/80 max-w-md leading-relaxed">
          {p.subtitle}
        </div>
        <div className="mt-4 flex gap-2 flex-wrap">
          {p.tags.map((t: string) => (
            <span
              key={t}
              className="micro text-white/60 border border-white/30 px-2 py-1 rounded-full"
            >
              {t}
            </span>
          ))}
...
export default function Featured() {
  ...
  return (
    <section className="px-6 md:px-10 pt-28 md:pt-40">
      <div
        ref={headReveal}
        className="flex items-end justify-between mb-16 md:mb-20 gap-8 flex-wrap"
      >
        <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-10">
        {featured.map((p: (typeof PROJECTS)[number], i: number) => (
